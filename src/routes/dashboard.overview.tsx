...
function CustomersDashboard() {
  ...
  const statusSegments = useMemo(() => {
    ...
    return [
      { label: "פעיל", value: stats.active, pct: (stats.active / total) * 100, color: "#10b981" },
      { label: "לא פעיל", value: stats.inactive, pct: (stats.inactive / total) * 100, color: "#94a3b8" },
      { label: "בהקפאה", value: stats.frozen, pct: (stats.frozen / total) * 100, color: "#38bdf8" },
    ].filter((s) => s.value > 0);
  }, [stats]);
    ...
    <div dir="rtl" className="space-y-6">
      ...
      <div className="flex flex-wrap items-start justify-between gap-4">
        <div className="flex items-center gap-3">
          ...
          <div>
            <h1 className="text-2xl font-bold tracking-tight">הלקוחות שלנו</h1>
            <p className="text-sm text-muted-foreground">
              ריכזו את כל הלקוחות שלכם במקום אחד
            </p>
          </div>
        </div>
      ...
      <div className="grid gap-4 lg:grid-cols-3">
        ...
        <Card className="p-5">
          <h3 className="mb-4 text-right font-semibold">פילוח לפי סטטוס לקוח</h3>
          {stats.total === 0 ? (
                ...
                {statusSegments.map((s) => (
                  <div key={s.label} className="flex items-center justify-between text-sm">
                    ...
                    <div className="flex items-center gap-2">
                      <span>{STATUS_LABEL[s.label] ?? s.label}</span>
                      <span
