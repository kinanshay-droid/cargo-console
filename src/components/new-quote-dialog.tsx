...
import {
  ...
  type PackageModel,
  type PackageMatch,
} from "@/lib/packaging-recommendation";
...
export const CARGO_TYPES: {
...
}[] = [
  {
    ...
    en: "General Cargo",
    icon: Package,
    tint: TONE_GRADIENT.muted,
  },
  {
    id: "temperature",
    label: "מטען מבוקר טמפ'",
    en: "Temperature Controlled",
    icon: Thermometer,
    tint: TONE_GRADIENT.accent,
  },
  ...
  {
    id: "dangerous",
    label: "סחורות מסוכנות",
    en: "Dangerous Goods",
    icon: AlertTriangle,
    tint: TONE_GRADIENT.destructive,
  },
...
const TEMP_SERIES_VISUAL: Record<
  TempSeriesKey,
  { icon: typeof Snowflake; tone: keyof typeof TONE_GRADIENT }
> = {
...
export const TEMP_LOGGERS: TempLogger[] = [
  ...
  },
  {
    ...
    model: "Tive Tag",
    type: "Passive / NFC",
    tempRange: "-30°C עד +50°C",
    gps: "❌",
    pharma: "⭐⭐⭐⭐",
  },
  {
    id: "sensitech-temptale",
    company: "Sensitech",
    model: "TempTale Ultra / TT4",
    type: "Single-use / USB",
    tempRange: "לפי דגם",
    gps: "❌ / חלק מהדגמים",
    pharma: "⭐⭐⭐⭐⭐",
  },
  {
    id: "elpro-libero",
    company: "ELPRO",
    model: "LIBERO CS / CB / CL",
    type: "Multi-use / PDF",
    tempRange: "לפי דגם",
    gps: "❌ / Cloud בדגמים מסוימים",
    pharma: "⭐⭐⭐⭐⭐",
  },
  {
    id: "logtag-trix",
    company: "LogTag",
    model: "TRIX-8 / SRIC / Dry Ice",
    type: "Single/Multi-use",
    tempRange: "לפי דגם",
    gps: "❌",
    pharma: "⭐⭐⭐⭐",
  },
  {
    id: "berlinger-qtag",
    company: "Berlinger",
    model: "Q-tag / Fridge-tag",
    type: "Single-use / Indicator",
    tempRange: "לפי דגם",
    gps: "❌",
    pharma: "⭐⭐⭐⭐⭐",
  },
  {
    ...
    type: "Multi-use",
    tempRange: "לפי דגם",
    gps: "❌ / Bluetooth בחלק מהדגמים",
    pharma: "⭐⭐⭐⭐",
  },
  {
    id: "dickson-one",
    company: "Dickson",
    model: "One / Data Loggers",
    type: "Multi-use / Cloud",
    tempRange: "לפי דגם",
    gps: "✅ בדגמי Cloud",
    pharma: "⭐⭐⭐⭐",
  },
  {
    id: "deltatrak-flashlink",
    company: "DeltaTrak",
    model: "FlashLink / ColdTrak",
    type: "Single/Multi-use",
    tempRange: "לפי דגם",
    gps: "חלק מהדגמים",
    pharma: "⭐⭐⭐⭐",
  },
  {
    id: "spotsee-warmmark",
    company: "SpotSee",
    model: "WarmMark / ColdMark",
    type: "Indicator",
    tempRange: "טווחים מוגדרים",
    gps: "❌",
    pharma: "⭐⭐⭐",
  },
  {
...
export function getPackModelCalc(sel: PackSelection, isImportShipment = false) {
  ...
  if (!m)
    return {
      label: modelName,
      qty: sel.qty,
      grossWeight: sel.qty * (productWeight + dryIceQty),
      volumetricWeight: 0,
      dims: null as { length: number; width: number; height: number } | null,
    };
  ...
  return {
    label: m.model,
    qty: sel.qty,
    grossWeight: sel.qty * (tare + productWeight + dryIceQty),
    volumetricWeight,
    dims,
  };
}
...
export const PALLETS = [
  {
    id: "eur",
    label: "משטח יורו (EUR/EPAL)",
    size: "120 × 80 × 14.4 ס״מ",
    length: 120,
    width: 80,
    height: 14.4,
  },
...
export type PackageRow = {
  ...
  loggerId: string | null;
  // Dry ice per unit (kg), only meaningful/shown when tempSeries is
  // "deepFrozen" — mirrors the dry-ice field on the catalog BioTherm table,
  // but for a manually-dimensioned box/pallet that isn't from that catalog.
  dryIceQty: string;
};
...
export function getPackageDimsCm(
  pkg: PackageRow,
  ...
  if (!preset || preset.length == null || preset.width == null || preset.height == null)
    return null;
...
export function getPackageCalc(pkg: PackageRow) {
  ...
  // Only counted for Deep Frozen (where the field is shown) — a leftover
  // value on a package later switched away from Deep Frozen is ignored.
  const dryIceQty = pkg.tempSeries === "deepFrozen" ? parseFloat(pkg.dryIceQty) || 0 : 0;
  ...
  const grossWeight = qty * (unitWeight + dryIceQty);
  const volumetricWeight = dims
    ? (qty * dims.length * dims.width * dims.height) / VOLUMETRIC_DIVISOR_CM3_PER_KG
    : 0;
  return { qty, unitWeight, dryIceQty, grossWeight, volumetricWeight, dims };
}
...
const COOLGUARD_CATALOG: PackageModel[] = COOLGUARD_MODELS.map((m, idx) => {
  const [internalLength, internalWidth, internalHeight] = m.inner
    .split("×")
...
export function NewQuoteDialog({
  ...
  const [cargo, setCargo] = useState<CargoRow[]>([
    { id: uid(), sku: "", description: "", packaging: "", weight: "", notes: "" },
  ]);
  const [containers, setContainers] = useState<ContainerRow[]>([
    { id: uid(), type: "", sku: "", destination: "", weight: "" },
  ]);
  const [goods, setGoods] = useState<GoodsRow[]>([
    { id: uid(), item: "", sku: "", origin: "", weight: "", dims: "", qty: 1 },
  ]);
  ...
  const removePickupContact = (id: string) =>
    setPickupContacts((arr) => (arr.length > 1 ? arr.filter((c) => c.id !== id) : arr));
  ...
  const removeDeliveryContact = (id: string) =>
    setDeliveryContacts((arr) => (arr.length > 1 ? arr.filter((c) => c.id !== id) : arr));
  ...
  const getPackProductWeight = (key: string) =>
    packSelections.find((s) => s.key === key)?.productWeight ?? "";
  const setPackProductWeight = (key: string, productWeight: number) =>
    setPackSelections((arr) => arr.map((s) => (s.key === key ? { ...s, productWeight } : s)));
  const getPackLogger = (key: string) =>
    packSelections.find((s) => s.key === key)?.loggerId ?? null;
  ...
  const getPackHasLogger = (key: string) =>
    packSelections.find((s) => s.key === key)?.hasLogger ?? null;
  const setPackHasLogger = (key: string, hasLogger: boolean) =>
    setPackSelections((arr) =>
      arr.map((s) =>
        s.key === key
          ? { ...s, hasLogger, loggerDataRead: hasLogger ? s.loggerDataRead : false }
          : s,
      ),
    );
  const getPackLoggerDataRead = (key: string) =>
    packSelections.find((s) => s.key === key)?.loggerDataRead ?? false;
  ...
  const getPackDryIceQty = (key: string) =>
    packSelections.find((s) => s.key === key)?.dryIceQty ?? "";
  ...
  const removePackage = (id: string) =>
    setPackages((rows) => (rows.length > 1 ? rows.filter((r) => r.id !== id) : rows));
  const packageCalcs = useMemo(
    () => packages.map((pkg) => ({ id: pkg.id, ...getPackageCalc(pkg) })),
    [packages],
  );
  ...
  const journeyMode = "air" as const;
  ...
  const [pricingItems, setPricingItems] = useState<PricingItem[]>(() =>
    DEFAULT_PRICING_ITEMS.map((r) => ({ ...r, price: 0 })),
  );
  ...
  useEffect(() => {
    ...
    const syncGroup = (
      ...
      const template =
        ...
        DEFAULT_PRICING_ITEMS.find((r) => r.group === group)!;
      const insertAt = rows.findIndex((r) => r.group === group);
      const groupRows = entries.map((e) => {
        const id = `${idPrefix}${e.key}`;
        const existing = byId.get(id);
        return existing
          ? { ...existing, sourceLabel: e.label }
          : { ...template, id, sourceLabel: e.label, price: 0 };
      });
  ...
  const STATUS_LABEL_DB: Record<string, string> = {
    active: "פעיל",
    inactive: "לא פעיל",
    frozen: "מוקפא",
  };
  ...
  const missingFieldLabels = useMemo(() => {
    ...
    if (step === 3) {
      ...
      if (cargoType === "temperature") {
        ...
        if (tempSeriesList.length > 0 && packSelections.length === 0)
          missing.push("בחירת אריזה מהקטלוג");
      }
  ...
  const step2SummaryLines = useMemo(() => {
    ...
    if (kind)
      lines.push({
        label: "סוג משלוח",
        value: SHIP_TYPES.find((t) => t.id === kind)?.label ?? kind,
      });
    if (kind !== "domestic" && incoterm) lines.push({ label: "תנאי מכר", value: incoterm });
    if (shipmentTypeTags.length > 0)
      lines.push({ label: "תגיות סוג משלוח", value: shipmentTypeTags.join(" · ") });
    ...
    if (houseBlNumber.trim())
      lines.push({ label: "מספר שטר מטען פנימי", value: houseBlNumber.trim() });
    if (kind === "import") {
      const journeyBits = [
        journeyOriginPort && journeyDestPort
          ? `${journeyOriginPort} → ${journeyDestPort}`
          : journeyOriginPort || journeyDestPort,
        airline.trim(),
    ...
    if (pickupNames.length > 0)
      lines.push({ label: "איש קשר באיסוף", value: pickupNames.join(" · ") });
    ...
    if (deliveryNames.length > 0)
      lines.push({ label: "איש קשר במסירה", value: deliveryNames.join(" · ") });
        ...
        <div ref={stepScrollRef} className="max-h-[60vh] overflow-y-auto p-6">
            ...
            <div className="space-y-6">
              ...
              {kind && (
                <Section
                  title="סוג משלוח *"
                  action={
                    <span className="text-xs text-muted-foreground">ניתן לבחור יותר מאחד</span>
                  }
                >
                      ...
                      <>
                        ...
                        {dropType && selectedSpec && (
                          <div className="mt-3 space-y-3 rounded-lg border bg-muted/20 p-3">
                            ...
                            <StopsEditor dropType={dropType} stops={stops} onChange={setStops} />
                          </div>
                        )}
                      </>
                    );
              ...
              {kind === "import" && (
                <Section title="ה. פרטי מסע *">
                  <div className="grid grid-cols-1 gap-3 md:grid-cols-2 lg:grid-cols-3">
                    <AirportCombobox
                      label="נמל מוצא *"
                      value={journeyOriginPort}
                      onChange={setJourneyOriginPort}
                    />
                    <AirportCombobox
                      label="נמל יעד *"
                      value={journeyDestPort}
                      onChange={setJourneyDestPort}
                    />
                      ...
                      <div key={c.id} className={cn("space-y-2", idx > 0 && "border-t pt-3")}>
                        ...
                        <div className="grid grid-cols-1 gap-2 sm:grid-cols-3">
                          <Field
                            label="שם *"
                            value={c.name}
                            onChange={(e) => updatePickupContact(c.id, { name: e.target.value })}
                          />
                          <Field
                            label="טלפון"
                            type="tel"
                            value={c.phone}
                            onChange={(e) => updatePickupContact(c.id, { phone: e.target.value })}
                          />
                          <Field
                            label='דוא"ל'
                            type="email"
                            value={c.email}
                            onChange={(e) => updatePickupContact(c.id, { email: e.target.value })}
                          />
                        </div>
                      </div>
                    ))}
                      ...
                      <div key={c.id} className={cn("space-y-2", idx > 0 && "border-t pt-3")}>
                        ...
                        <div className="grid grid-cols-1 gap-2 sm:grid-cols-3">
                          <Field
                            label="שם *"
                            value={c.name}
                            onChange={(e) => updateDeliveryContact(c.id, { name: e.target.value })}
                          />
                          <Field
                            label="טלפון"
                            type="tel"
                            value={c.phone}
                            onChange={(e) => updateDeliveryContact(c.id, { phone: e.target.value })}
                          />
                          <Field
                            label='דוא"ל'
                            type="email"
                            value={c.email}
                            onChange={(e) => updateDeliveryContact(c.id, { email: e.target.value })}
                          />
                        </div>
                      </div>
                    ))}
              ...
              <Section title="הערות">
                <Textarea
                  ...
                  placeholder="הערות כלליות למשלוח…"
                  rows={3}
                />
              </Section>
            </div>
            ...
            <div className="space-y-6">
              <Section
                ...
                action={
                  <span className="text-xs text-muted-foreground">
                    בחר את סוג המטען שברצונך לשלוח
                  </span>
                }
              >
                  ...
                  {CARGO_TYPES.map((t) => {
                    ...
                    return (
                      ...
                      >
                        <div
                          className={cn(
                            "mb-3 inline-flex h-10 w-10 items-center justify-center rounded-lg bg-gradient-to-br text-white shadow-sm",
                            t.tint,
                          )}
                        >
                          <Icon className="h-5 w-5" />
              ...
              {cargoType === "temperature" && (
                <Section
                  ...
                  action={
                    <span className="text-xs text-muted-foreground">
                      אפשר לבחור כמה סדרות טמפרטורה במקביל
                    </span>
                  }
                >
                    ...
                    {TEMP_SERIES.map((s) => {
                      ...
                      return (
                        ...
                        >
                          <div
                            className={cn(
                              "mb-2 inline-flex h-9 w-9 items-center justify-center rounded-lg bg-gradient-to-br text-white",
                              TONE_GRADIENT[visual.tone],
                            )}
                          >
                            <Icon className="h-4.5 w-4.5" />
                          </div>
                          ...
                          <div key={series} className="overflow-x-auto rounded-lg border">
                            <div className="flex items-center justify-between border-b bg-muted/20 px-3 py-1.5">
                              <span className="text-xs font-medium">
                                {seriesInfo?.icon} {seriesInfo?.label}{" "}
                                <span className="text-muted-foreground">({seriesInfo?.range})</span>
                              </span>
                              <span className="text-[11px] text-muted-foreground">
                                אפשר לבחור כמה סוגי אריזה שונים, כל אחד בכמות משלו
                              </span>
                            </div>
                            {series === "deepFrozen" ? (
                                  ...
                                  {BIOTHERM_MODELS.map((m) => {
                                    ...
                                    return (
                                      ...
                                      >
                                        ...
                                        <td className="px-3 py-2 text-muted-foreground">
                                          {calc
                                            ? `${calc.volumetricWeight.toLocaleString("he-IL", { maximumFractionDigits: 2 })} ק"ג`
                                            : ""}
                                        </td>
                                ...
                                <tbody>
                                  {COOLGUARD_MODELS.map((m) => {
                                    ...
                                    const calc =
                                      qty > 0
                                        ? getPackModelCalc(
                                            { key, qty, productWeight: Number(productWeight) || 0 },
                                            kind === "import",
                                          )
                                        : null;
                                    return (
                    ...
                    <div key={pkg.id} className={cn("relative", idx > 0 && "border-t pt-4")}>
                      ...
                      <div className="mt-4 max-w-xs">
                        ...
                        <div className="mt-1.5">
                          <LoggerPicker
                            value={pkg.loggerId}
                            onChange={(id) => updatePackage(pkg.id, { loggerId: id })}
                          />
                        </div>
                      </div>
                      {cargoType === "temperature" &&
                        (() => {
                          ...
                          const tempLabel =
                            tempSeriesList.length > 0
                              ? tempSeriesList
                                  .map((k) => TEMP_SERIES.find((t) => t.key === k)?.label)
                                  .filter(Boolean)
                                  .join(" + ")
                          ...
                          return (
                            <div className="mt-3 rounded-lg border bg-muted/20 p-3">
                              <div className="mb-2 space-y-0.5">
                                ...
                                <div className="text-xs text-muted-foreground">
                                  סוג מארז:{" "}
                                  <span className="font-medium text-foreground">{palletLabel}</span>
                              ...
                              {matches.length === 0 ? (
                                <p className="text-xs text-muted-foreground">
                                  לא נמצאה אריזה מהקטלוג שמתאימה למידות אלו.
                                </p>
                              ) : (
                                <div className="space-y-1.5">
                                  {matches.slice(0, 3).map((m, rank) => (
                                    ...
                                    >
                                      ...
                                      <span className="text-muted-foreground">
                                        סבב {m.rotation} · ניצול{" "}
                                        {(m.utilization * 100).toLocaleString("he-IL", {
              ...
              <Section title="סיכום משקלי ונפחי">
                <div className="overflow-hidden rounded-lg border">
                  <table className="w-full text-sm">
                    ...
                    <tbody className="divide-y">
                      {packageCalcs.map((c, idx) => {
                        ...
                        return (
                          <tr key={c.id}>
                            ...
                            <td className="px-3 py-2">
                              {c.grossWeight ? `${c.grossWeight.toLocaleString("he-IL")} ק"ג` : ""}
                            </td>
                            <td className="px-3 py-2">
                              {c.volumetricWeight
                                ? `${c.volumetricWeight.toLocaleString("he-IL", { maximumFractionDigits: 2 })} ק"ג`
                                : ""}
                            </td>
                          </tr>
                        );
                      ...
                      {packageCalcs.every((c) => !c.dims && !c.grossWeight) &&
                        packModelCalcs.length === 0 && (
                          <tr>
                            <td
                              colSpan={5}
                ...
                <div className="mt-3 grid grid-cols-1 gap-3 sm:grid-cols-3">
                  <div className="rounded-lg border bg-muted/20 p-3 text-center">
                    ...
                    <div className="mt-1 text-lg font-semibold">
                      {packageTotals.grossWeight.toLocaleString("he-IL")} ק"ג
                    </div>
                  </div>
                  <div className="rounded-lg border bg-muted/20 p-3 text-center">
                    <div className="text-[11px] text-muted-foreground">סה״כ משקל נפחי</div>
                    <div className="mt-1 text-lg font-semibold">
                      {packageTotals.volumetricWeight.toLocaleString("he-IL", {
                  ...
                  <div className="rounded-lg border border-primary bg-primary/5 p-3 text-center">
                    <div className="text-[11px] text-muted-foreground">
                      משקל חייב (הגבוה מבין השניים)
                    </div>
                    <div className="mt-1 text-lg font-semibold text-primary">
                      {packageTotals.chargeableWeight.toLocaleString("he-IL", {
                        maximumFractionDigits: 2,
                      })}{" "}
                      ק"ג
                    </div>
                ...
                <p className="mt-2 text-[11px] text-muted-foreground">
                  חישוב המשקל הנפחי מבוסס על מקדם IATA לתובלה אווירית: (אורך × רוחב × גובה בס״מ) ÷
                  6000.
                </p>
              </Section>
              ...
              <div className="grid grid-cols-1 gap-4 sm:grid-cols-2">
                <Section title="דרישות מיוחדות">
                  <Textarea
                    ...
                    placeholder="הזן דרישות מיוחדות למשלוח…"
                    rows={3}
                  />
                </Section>
                <Section title="הערות">
          ...
          {step === 4 && (
            <Step4Logistics
              ...
              setLogisticsNotes={setLogisticsNotes}
              routeApproved={routeApproved}
              setRouteApproved={setRouteApproved}
          ...
          {step === 5 && (
            <Step5Pricing
              ...
              notes={pricingNotes}
              setNotes={setPricingNotes}
              dismissed={dismissedAlerts}
              setDismissed={setDismissedAlerts}
          ...
          <div className="flex items-center gap-2">
            ...
            {!canContinue && !submitting && missingFieldLabels.length > 0 && (
              <span className="max-w-md text-xs text-muted-foreground">
                <span className="font-medium text-foreground">חסר למילוי:</span>{" "}
...
function StopField({
  ...
  const type =
    field === "etaAt" || field === "ataAt" ? "datetime-local" : field === "phone" ? "tel" : "text";
  return (
    <div className="space-y-1.5">
      <Label className="text-xs text-muted-foreground">{label}</Label>
        ...
        >
          <span className="truncate">
            {selected ? (
              ...
              <span className="text-muted-foreground">בחר רשם...</span>
            )}
          </span>
            ...
            <tbody className="divide-y">
              {TEMP_LOGGERS.map((l) => (
                ...
                >
                  ...
                  <td className="px-2.5 py-2 text-muted-foreground" dir="ltr">
                    {l.tempRange}
                  </td>
  ...
  return (
    <div className="inline-flex items-center rounded-md border">
      <button
        ...
        className="px-2 py-1 text-sm hover:bg-muted/50"
      >
        −
      </button>
...
type Step4Props = {
  ...
  transit: string[];
  setTransit: React.Dispatch<React.SetStateAction<string[]>>;
  newTransit: string;
  setNewTransit: (v: string) => void;
  departDate: string;
  arriveDate: string;
  services: Record<string, boolean>;
  setServices: React.Dispatch<React.SetStateAction<Record<string, boolean>>>;
  compare: Record<string, boolean>;
  setCompare: React.Dispatch<React.SetStateAction<Record<string, boolean>>>;
  agent: string;
  setAgent: (v: string) => void;
  agents: string[];
  setAgents: React.Dispatch<React.SetStateAction<string[]>>;
  agentRoles: Record<string, "pickup" | "delivery">;
  setAgentRoles: React.Dispatch<React.SetStateAction<Record<string, "pickup" | "delivery">>>;
  airline: string;
  setAirline: (v: string) => void;
  logisticsNotes: string;
  setLogisticsNotes: (v: string) => void;
  routeApproved: boolean;
  setRouteApproved: (v: boolean) => void;
  shipmentMode: ShipmentMode | null;
  setShipmentMode: (v: ShipmentMode) => void;
};

function Step4Logistics(p: Step4Props) {
  const transitDays = useMemo(() => {
    const d1 = new Date(p.departDate);
    const d2 = new Date(p.arriveDate);
  ...
  return (
    <div className="space-y-6">
      <Section
        title="מסלול מוצע"
        action={<span className="text-xs text-muted-foreground">בחר את תוכנית המשלוח המועדפת</span>}
      >
        <div>
        ...
        <div className="mt-4">
          ...
          <div className="mt-2 flex flex-wrap items-center gap-2">
            {p.transit.map((t) => (
              ...
              >
                ...
                <button
                  type="button"
                  onClick={() => p.setTransit((arr) => arr.filter((x) => x !== t))}
            ...
            <div className="flex items-center gap-1">
              <Input
                value={p.newTransit}
                onChange={(e) => p.setNewTransit(e.target.value.toUpperCase())}
                placeholder="קוד נמל"
              ...
              <Button
                ...
                className="h-8 gap-1"
                onClick={() => {
                  if (p.newTransit.trim()) {
                    p.setTransit((a) => [...a, p.newTransit.trim()]);
                    p.setNewTransit("");
                  }
                }}
              >
          ...
          <div className="mt-1 text-[11px] text-muted-foreground">
            ניתן להוסיף נמלי ביניים על מנת לתכנן מסלול עם עצירות
          </div>
        </div>
            ...
            return (
              <button
                ...
                className={cn(
                  ...
                  on ? "border-primary bg-primary/5" : "hover:bg-muted/30 text-muted-foreground",
                )}
              >
                <div
                  className={cn(
      ...
      <div className="grid grid-cols-1 gap-4 sm:grid-cols-2">
        ...
        >
          {isDropShipment ? (
            <>
                      ...
                      >
                        ...
                        <span className="flex items-center gap-0.5 rounded-full border bg-background p-0.5">
                          <button
                            type="button"
                            onClick={() =>
                              p.setAgentRoles((r) => ({ ...r, [item.code]: "pickup" }))
                            }
                            className={cn(
                          ...
                          <button
                            type="button"
                            onClick={() =>
                              p.setAgentRoles((r) => ({ ...r, [item.code]: "delivery" }))
                            }
                            className={cn(
                              "rounded-full px-2 py-0.5 text-[11px] font-medium transition-colors",
                              role === "delivery"
              ...
              {p.agents.length === 0 && (
                <p className="mt-2 text-[11px] text-muted-foreground">
                  ניתן לבחור יותר מסוכן אחד למשלוחי דרופ (איסוף מסוכן אחד, מסירה לסוכן אחר וכו׳).
                </p>
              )}
            </>
          ) : (
            <>
              ...
              <ul className="mt-3 space-y-1 text-xs text-muted-foreground">
                <li>
                  סוג שירות: <span className="font-medium text-foreground">Premium</span>
                </li>
                <li>
                  סניף מטפל: <span className="font-medium text-foreground">TLV Office</span>
                </li>
                <li>
                  איש קשר: <span className="font-medium text-foreground">John Doe</span>
                </li>
                <li>
                  טלפון: <span className="font-medium text-foreground">+972-3-1234567</span>
                </li>
                <li>
                  דוא״ל: <span className="font-medium text-foreground">tlv@quickstat.com</span>
                </li>
              </ul>
            </>
          )}
        ...
        <Section title="חברת תעופה מתוכננת">
          <Field
            label="שם חברת התעופה"
            value={p.airline}
            onChange={(e) => p.setAirline(e.target.value)}
          />
          <ul className="mt-3 space-y-1 text-xs text-muted-foreground">
            <li>
              מספר טיסה: <span className="font-medium text-foreground">LH 401</span>
            </li>
            <li>
              מסלול: <span className="font-medium text-foreground">JFK → FRA → TLV</span>
            </li>
            <li>
              תדירות: <span className="font-medium text-foreground">יומי</span>
            </li>
            <li>
              סוג מטוס: <span className="font-medium text-foreground">B777F</span>
            </li>
            <li>
              יציאה משוערת: <span className="font-medium text-foreground">10/06/2025 22:30</span>
            </li>
            <li>
              הגעה משוערת: <span className="font-medium text-foreground">11/06/2025 18:15</span>
            </li>
          </ul>
        </Section>
      </div>
      ...
      <Section title="הערות תכנון לוגיסטי *">
        <Textarea
          value={p.logisticsNotes}
          onChange={(e) => p.setLogisticsNotes(e.target.value)}
          maxLength={500}
          rows={3}
          placeholder="הזן הערות לגבי תכנון הלוגיסטיקה..."
        />
        <div className="mt-1 text-left text-[11px] text-muted-foreground">
          {p.logisticsNotes.length}/500
        </div>
      </Section>
      ...
      <label className="flex items-center gap-2 rounded-lg border bg-muted/20 p-3 text-sm">
        <input
          type="checkbox"
          checked={p.routeApproved}
          onChange={(e) => p.setRouteApproved(e.target.checked)}
...
export const DEFAULT_PRICING_ITEMS: PricingItem[] = [
  ...
  },
  {
    ...
    label: "טיפול נמל יעד",
    source: "pricelist",
    sourceLabel: "Terminal Handling (Destination)",
    sourceDate: "2025-05-20",
    currency: "USD",
    price: 85,
  },
  ...
  },
  {
    ...
    label: "אריזה חיוונית",
    source: "pricelist",
    sourceLabel: "Packaging (CoolGuard 60L)",
    sourceDate: "2025-05-20",
    currency: "USD",
    price: 180,
  },
  {
    ...
    label: "רשם טמפרטורה",
    source: "pricelist",
    sourceLabel: "Temperature Logger",
    sourceDate: "2025-05-20",
    currency: "USD",
    price: 95,
  },
...
function Step5Pricing(p: Step5Props) {
  ...
  const removeRow = (id: string) => p.setItems((rows) => rows.filter((r) => r.id !== id));
    ...
    <div className="space-y-4">
      ...
      <div className="flex flex-wrap items-center justify-between gap-3 rounded-xl border bg-card p-3">
        ...
        <SummaryChip
          icon={Route}
          label="מסלול"
          value={`${extractCode(p.originPort)} → ${extractCode(p.destPort)}`}
          sub={p.departDate}
        />
        ...
        <div className="space-y-4 lg:col-span-2">
          <Section
            title={
              (
                <span className="flex items-center gap-2">
                  פריטים ועלויות <Info className="h-3.5 w-3.5 text-muted-foreground" />
                </span>
            ...
            action={
              <div className="flex items-center gap-2">
                ...
                <select
                  value={p.currency}
                  onChange={(e) => p.setCurrency(e.target.value as "USD" | "EUR" | "ILS")}
          ...
          >
                ...
                <tbody>
                  {p.items.map((it) => {
                    ...
                    return (
                      <tr
                        key={it.id}
                        className={cn("border-t", meta.row, it.stale && "bg-warning/10/60")}
                      >
                        ...
                        <td className="px-2 py-2">
                          <input
                            ...
                            onChange={(e) =>
                              p.setItems((rows) =>
                                rows.map((r) =>
                                  r.id === it.id ? { ...r, price: Number(e.target.value) || 0 } : r,
                                ),
                              )
                            }
                            placeholder="0"
                            className={cn(
                              "h-8 w-24 rounded-md border bg-background px-2 text-right text-sm",
                              it.source === "missing" && "border-destructive/40 text-destructive",
                            )}
                          />
                        </td>
                          ...
                          <select
                            ...
                            onChange={(e) =>
                              p.setItems((rows) =>
                                rows.map((r) =>
                                  r.id === it.id
                                    ? { ...r, currency: e.target.value as "USD" | "EUR" | "ILS" }
                                    : r,
                                ),
                              )
                            }
                        ...
                        <td className="px-3 py-2 text-xs">
                          <div className="flex items-center justify-end gap-2">
                            ...
                            <span
                              className={cn(
                                it.source === "missing" && "text-destructive",
                                it.stale && "text-warning",
                              )}
                            >
                              {it.sourceLabel}
                            </span>
                          ...
                          {it.source === "manual" ? (
                            <input
                              ...
                              onChange={(e) => {
                                ...
                                p.setItems((rows) =>
                                  rows.map((r) =>
                                    r.id === it.id ? { ...r, label: name, group: name } : r,
                                  ),
                                );
            ...
            <div className="mt-3 flex flex-wrap items-center justify-center gap-2">
              ...
              >
                ...
                <PopoverContent align="center" className="w-[440px] p-0">
                  <div className="border-b px-4 py-3">
                    ...
                    <div className="text-xs text-muted-foreground">
                      בחר מהרשימה את הפריטים שברצונך להוסיף להצעת המחיר
                    </div>
                  </div>
                    ...
                    {costGroupCatalog.map((item) => {
                      ...
                      return (
                        ...
                        >
                          ...
                          {alreadyAdded && (
                            <span className="text-[11px] text-muted-foreground">כבר נוסף</span>
                          )}
                        </button>
                      );
                    })}
                  ...
                  <div className="flex items-center justify-between gap-3 border-t bg-muted/30 px-4 py-2.5">
                    <span className="text-xs text-muted-foreground">
                      {pickedCostGroupIds.length > 0
                        ? `${pickedCostGroupIds.length} פריטים נבחרו`
                        : "לא נבחרו פריטים"}
                    </span>
                    <Button
                      type="button"
                      ...
                      onClick={() => {
                        const toAdd = costGroupCatalog.filter((item) =>
                          pickedCostGroupIds.includes(item.id),
                        );
                        p.setItems((rows) => [
                          ...rows,
              ...
              <Button
                ...
                size="sm"
                variant="ghost"
                disabled={p.items.every(
                  (r) => r.group === "אריזה חיוונית" || r.group === "רשם טמפרטורה",
                )}
                ...
                onClick={() => {
                  ...
                  const clearable = p.items.filter(
                    (r) => r.group !== "אריזה חיוונית" && r.group !== "רשם טמפרטורה",
                  );
                  if (clearable.length === 0) return;
                  if (
                    !window.confirm(
                      "למחוק את שורות העלויות מהטבלה (מלבד אריזה ורשם שנבחרו בשלב 3)? לא ניתן לשחזר לאחר השמירה.",
                    )
                  )
                    return;
                  p.setItems((rows) =>
                    rows.filter((r) => r.group === "אריזה חיוונית" || r.group === "רשם טמפרטורה"),
                  );
        ...
        <div className="space-y-4">
          <Section
            title={
              (
                <span className="flex items-center gap-2">
                  סיכום מחיר <Info className="h-3.5 w-3.5 text-muted-foreground" />
                </span>
              ) as unknown as string
            }
            action={
            ...
            <div className="space-y-2 text-sm">
              ...
              <div className="flex items-center justify-between rounded-md bg-muted/40 px-3 py-2">
                ...
                <input
                  type="number"
                  value={p.margin}
                  onChange={(e) => p.setMargin(e.target.value)}
            ...
            <div className="flex items-start gap-2">
              ...
              <div className="text-xs">
                ...
                <div className="text-destructive/80">
                  {alerts.filter((a) => a.tone === "rose").length} קריטי ·{" "}
                  {alerts.filter((a) => a.tone === "amber").length} אזהרה · {p.items.length} פריטים
                  · שיעור רווח {profitRate.toFixed(1)}%
                </div>
              </div>
            </div>
          </div>
          ...
          <Section
            title={
              (
                <span>
                  התראות והמלצות{" "}
                  <span className="mr-1 text-xs text-muted-foreground">{alerts.length} סה״כ</span>
                </span>
              ) as unknown as string
            }
          >
                  ...
                  <div className="mb-1 flex items-center justify-between gap-2">
                    <div
                      className={cn(
                        ...
                        a.tone === "rose" ? "text-destructive" : "text-warning",
                      )}
                    >
      ...
      <Section title="הערות">
        <Textarea
          ...
          placeholder="הזן הערות להצעת המחיר…"
          rows={3}
        />
    ...
    <div className="space-y-4" data-testid="wizard-step-6">
      ...
      <div className="rounded-xl border bg-card p-4">
        ...
        <div className="grid grid-cols-1 gap-4 md:grid-cols-2">
          <div>
            ...
            <div className="text-xs text-muted-foreground">{p.customer?.customer_code ?? ""}</div>
          </div>
          <div className="text-sm">
      ...
      <div className="rounded-xl border bg-card p-4">
        ...
        <div className="mt-2 text-[11px] text-muted-foreground">
          * הסכומים הכוללים (רווח/הנחה/סה״כ להצעה) מחושבים על בסיס {p.currency} בלבד — פריטים
          במטבעות אחרים מוצגים בנפרד ואינם נכללים בחישוב
        </div>
        <div className="text-[11px] text-muted-foreground">
        ...
        <div className="rounded-xl border bg-card p-4">
          ...
          <ul className="mt-3 space-y-1.5 text-xs text-muted-foreground">
            <li className="flex gap-1.5">
              <Check className="h-3.5 w-3.5 text-success" /> כל התנאים מולאו
            </li>
            <li className="flex gap-1.5">
              <Check className="h-3.5 w-3.5 text-success" /> שיעור רווח תקין
            </li>
            <li className="flex gap-1.5">
              <Check className="h-3.5 w-3.5 text-success" /> אין חריגות
            </li>
            <li className="flex gap-1.5">
              <Check className="h-3.5 w-3.5 text-success" /> אישורי סוכן קיימים
            </li>
          </ul>
